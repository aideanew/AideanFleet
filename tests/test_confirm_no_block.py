...
def test_blocked_confirm_returns_immediately_and_redispatches_async(monkeypatch):
    # 确保数据库表已初始化（TestClient 在新版 FastAPI 中不自动触发 startup）
    from fleet.core import db as fleet_db
    fleet_db.init_db()
    # mock find_project 让登录通过（测试不依赖真实项目数据）
    monkeypatch.setattr(
        console_server.db, "find_project",
        lambda key: {"project_id": "AideanFleet", "name": "AideanFleet", "path": ""},
    )
    with TestClient(console_server.app) as tc:
        _login(tc)
        task_id = "EVICT-T01"
        task = {
